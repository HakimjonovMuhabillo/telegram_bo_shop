...
async def command_start(message: Message):
    await message.answer(f'Здравствуйте {message.from_user.full_name}, Вас приветствует бот')
...
async def remove_product_cart(call: CallbackQuery):
    ...
    cart_id = get_user_cart_id(chat_id)
    if quantity <= 1:
        await bot.answer_callback_query(call.id, 'Ниже 0 нельзя')
        pass
    else:
        quantity -= 1
        await bot.edit_message_caption(chat_id=chat_id, message_id=message_id,
...
async def show_cart(message: Message, edit_message: bool = False):
    ...
    Общее стоимость счёта: {0 if total_price is None else total_price} '''
...
async def show_history_orders(message: Message):
    chat_id = message.chat.id
    cart_id = get_user_cart_id(chat_id)
    order_total_price = get_orders_total_price(cart_id)
